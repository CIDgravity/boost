...
func dealCidGravityCmdAction(cctx *cli.Context) error {
	...
	if err != nil {
		return cmd.PrintJson(dealCidGravityResponse{
			Status:  Unavailable,
			Reason:  "ERR_OPEN_STREAM_TO_ADDRESS_FOR_GET_ASK",
	...
	if err != nil {
		return cmd.PrintJson(dealCidGravityResponse{
			Status:  InternalError,
			Reason:  "ERR_BUILDING_DEAL_PROPOSAL",
	...
	if err != nil {
		return cmd.PrintJson(dealCidGravityResponse{
			Status:  Unavailable,
			Reason:  "ERR_OPEN_STREAM_TO_PEER_ID",
	...
	return cmd.PrintJson(dealCidGravityResponse{
		Status:  Unkown,
		Reason:  "SENT",
		Message: "deal proposal successfully sent",
	})
}
